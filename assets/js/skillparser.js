	...
	static parse(locale, data, i) {
		...
		for (const fv of fixedvalues) {
			...
			if (data.fixedvalues[fv[1]] === undefined) {
				desc = desc.replace(`(fv:${fv[1]})`, "?")
			} else {
				desc = desc.replace(`(fv:${fv[1]})`, data.fixedvalues[fv[1]])
			}
		}
		...
		if (!this.EFFECTS) {
			...
			for (const ef of effects) {
				...
				if (dur === 0) desc = desc.replace(`{effect:${ef[1]}}`, toreplace + `${thiseffect.name}</i>`)
				else
					desc = desc.replace(
						`{effect:${ef[1]}}`,
		...
		if (!this.skills) desc = desc.replace(/{skill:[0-9]+}/, "?")
			...
			for (const sk of skills) {
				...
				desc = desc.replace(`{skill:${sk[1]}}`, toreplace)
			}
		}
